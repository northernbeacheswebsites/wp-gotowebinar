...
//           usersTimezone = data['time_zone'];
//           renderCalendar(usersTimezone); 
//        });    
//    }
...
if ($('input[name="gotowebinar_opt_in"]').prop('checked') == true || $('input[name="gotowebinar_opt_in"]').length == 0) {
    ...
    //make ajax request to activeCampaign
    var data = {
        'action': 'activecampaign_subscribe',
        'activeCampaignFirstName': firstName,
        'activeCampaignLastName': lastName,
        'activeCampaignEmail': email,
        'activeCampaignPhone': phone,
        'activeCampaignOrganization': organization,
        'activeCampaignList': activeCampaignList
    };
    jQuery.post(activecampaign_subscribe.ajaxurl, data);
